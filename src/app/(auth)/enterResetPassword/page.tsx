...
import { enterResetPassword } from "@/services/auth.services";

export default function EntertResetPassword() {
  ...
  async function handleResetPassword(values: resetPasswordTypeSchema) {
    const resposne = await enterResetPassword(values);
    if (resposne?.token) {
      ...
      toast.error(resposne?.message || "Something went wrong", {
        position: "top-center",
      });
    }
  }
